...
<script type="text/javascript">
	$(document).ready(function(){
		$(\'#tables\').before(\'<div class="filter-container"><input type="text" name="filter" placeholder="Filter tables..." class=""></div>\');
		$(\'body\').on(\'keyup\', \'.filter-container input[name="filter"]\', function(){
			if($(this).val()){
				$(this).parent().siblings(\'#tables\').children().hide();
				var $this = $(this);
				$(\'#tables .structure\').each(function(){
					if($(this).text().toLowerCase().indexOf($this.val().toLowerCase()) >= 0){
						$(this).parent().show();
					}
				});
			} else {
				$(this).parent().siblings(\'#tables\').children().show();
			}
		});
	});
</script>
...
as$y=>$X)$td[]="$y $X";}$b->headers();}function
csp(){return
